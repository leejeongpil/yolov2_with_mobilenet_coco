...
def depth_point_layer(x, side_l, stride, out_depth, padding='SAME', use_bias=True, **kwargs):
    """
    Add a new convolutional layer.
    :param x: tf.Tensor, shape: (N, H, W, C).
    :param side_l: int, the side length of the filters for each dimension.
    ...
    in_depth = int(x.get_shape()[-1])
    #filters = weight_variable([side_l, side_l, in_depth, out_depth], stddev=weights_stddev)
    filter_depth = tf.Variable(tf.truncated_normal([side_l, side_l, in_depth , 1], stddev=0.01))
    depth_conv = tf.nn.depthwise_conv2d(x, filter_depth, [1,1,1,1], padding='SAME')
    batch_depth = tf.layers.batch_normalization(depth_conv, training=True, momentum=0.99, epsilon=0.001, center=True, scale=True)
    leakyrelu_depth = tf.nn.leaky_relu(batch_depth, alpha=0.1)
    filter_point = weight_variable([1, 1, in_depth, out_depth], stddev=weights_stddev)
    return conv2d(leakyrelu_depth, filter_point, stride, padding=padding)
