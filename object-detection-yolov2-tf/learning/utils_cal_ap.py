...
def nms(boxes,  iou_thres=0.5, conf_thres=0.2):
    x1 = boxes[..., 0]
    y1 = boxes[..., 1]
    x2 = boxes[..., 2]
    y2 = boxes[..., 3]
...
def predict_nms_boxes(input_y, iou_thres=0.5, conf_thres=0.2):
    is_batch = len(input_y.shape) == 5
    if not is_batch:
        input_y = np.expand_dims(input_y, 0)
    boxes = np.reshape(input_y, (input_y.shape[0], -1, input_y.shape[-1]))
    nms_boxes = []
    for box in boxes:
        nms_box = nms(box, iou_thres, conf_thres)
...
def cal_accuracy(gt_bboxes, bboxes, iou_thres=0.5):
    p = 0
    tp = 0
...
def cal_precision(gt_bboxes, bboxes, iou_thres=0.5):
    tp = 0
    pp = 0
    for idx, (gt, bbox) in enumerate(zip(gt_bboxes, bboxes)):
        gt = gt[np.nonzero(np.any(gt > 0, axis=1))]
...
def cal_ap(gt_bboxes, bboxes, iou_thres=0.5):
    ap_list = []
    rec = []
    for i in range(0,11):
        conf_score = i/10
        ...
        for idx, (gt, bbox) in enumerate(zip(gt_bboxes, bboxes)):
            asd = []
            gt = gt[np.nonzero(np.any(gt > 0, axis=1))]
            for j in range(0, 845):
                if bbox[j, 5]>=conf_score:
                    asd.append(j)
            bbox = bbox[asd]
            # bbox = bbox[np.nonzero(np.any(bbox > 0, axis=1))]
